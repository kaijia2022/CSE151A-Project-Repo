...
## Jupyter Notebook
Below we have our Jupyter Notebook used to perform our research.
...
* **Star Type (numerical)** - Contains 6 star types which are brown type, red dwarf, white dwarf, main sequence, supergiants, hypergiants; which are then represented by a numerical value.
...
![alt text](images/image-2.png)
    *Figure displays each numerical feature on the x-axis with a corresponding feature on the y-axis with the different star types as different colored dots. The star type legend is shown on the right.*
...
3. We then used `pd.df.isna()` to look for missing data or null values. After running it we found there were no null values or missing data and as such we did not have have to take any further steps to deal with missingness.  

4. Finally, we concatenated the normalized data with the one hot encoded features and stored it as our final dataframe via `processd_df = pd.concat([normal, oneHot_color, oneHot_class, df['Star type']], axis=1)`

* Final Preprocessed Data:
![alt text](images/image-4.png)
    *Table shows the exact dataframe we use for our models.*
...
* **Model 1**:
    ...
    * Fitting Plot:
    ![Fitting plot](images/image.png)
        *Figure displays how the relationship between number of features, aka model complexity, and the mean squared error of the linear regression model. Blue shows the training error and orange shows the testing error.*
    ___
* **Model 2**:
    * Classification report: The classification report below give us a detailed breakdown of how well our model performance on each star type prediction.  
    ![alt text](images/image-8.png)
    * Accuracy Plot:The plot shows how our accuray rate changed during we train our testing data. 
    ![alt text](images/image-6.png)
    * Model Loss Plot: The plot shows how our loss rate changed during we train our testing data. 
